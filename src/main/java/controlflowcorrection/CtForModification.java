...
class CtForModification {
     ...
     * @return                  Returns an array of blocks where the first element contains the 'for' statement with the
     *                          condition modified and the body after the addition of the 'if'. While in the second
     *                          element will be a 'for' statement with the condition modified and the body after the
     *                          addition of the 'if' also modified.
     */
    ...
    /**
     * Discovers if it is necessary to add an 'if' statement.
     * @param stoppingCondition  The stopping condition of the 'for' statement.
     * @param body               The body of the 'for' statement.
     * @return                   Returns true if the 'if' statement is needed. False otherwise.
     */
    private static boolean isIfNecessary(CtExpression<Boolean> stoppingCondition, CtBlock<?> body) {
        if (body.getStatements().size() == 1) {
            if (body.getStatement(0) instanceof CtIf) {
                ...
                return !condition.toString().contains(stoppingCondition.toString());
            }
        }
        return true;
    }
     ...
     */
    private static boolean handleStoppingCondition(Factory factory,
                                                   List<CtVariable<?>> secretVariables,
                                                   List<CtParameter<?>> publicArguments,
                                                   CtFor forStatement,
     ...
     */
    private static CtBinaryOperator<Boolean> modifyStoppingCondition(Factory factory,
                                                                     List<CtVariable<?>> secretVariables,
                                                                     List<CtParameter<?>> publicArguments,
     ...
     */
    private static CtExpression<?> modifyOperand(Factory factory,
                                                 CtVariable<?> secretArgument,
                                                 List<CtParameter<?>> publicArguments,
                                                 CtExpression<?> handOperand) {
