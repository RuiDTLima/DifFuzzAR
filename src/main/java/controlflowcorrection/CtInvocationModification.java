...
public class CtInvocationModification {
	 ...
	 * @return	Returns an array of blocks where in the first index is a block with the original invocation and in the
	 * second index is a block with the modified invocation.
	 */
	static CtBlock<?>[] modifyInvocation(CtElement element, Factory factory, CtIfImpl initialStatement,
	...
	private static CtConstructorCall<?> createNewDefaultExpression(Factory factory, Set<String> dependableVariables,
		...
		for (CtExpression<?> argument : arguments) {
			Optional<CtVariable<?>> optionalVariable = secretVariables.stream().filter(secret -> secret.getSimpleName().equals(argument.toString())).findFirst();
			if (optionalVariable.isPresent()) {
